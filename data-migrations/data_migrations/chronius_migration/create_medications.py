        ...
        with open(self.csv_file, 'w') as f:
            ...
            with open(self.customer_file, 'r') as file:
                ...
                for row in reader:
                    ...
                    mapping_found = self.med_mapping.get(f"{medication_name}|{row['RxNorm/FDB Code']}".lower())
                    ...
                    writer.writerow({
                        "ID": f'{row["Patient Identifier"]}-{medication_name}',
                        "Patient Identifier": row['Patient Identifier'],
                        "Status": row['Status'].lower(),
                        "RxNorm/FDB Code": code,
                        "SIG": f"Name: {sig}",
                        "Medication Name": medication_name,
                        "Original Code": row['RxNorm/FDB Code']
                    })
...
if __name__ == '__main__':
    ...
    loader = MedicationLoader(environment='phi-chronius-test')
    delimiter = ','
...
# import csv
# with open("done_medications.csv", 'r') as f:
#     reader = csv.DictReader(f, delimiter='|')
#     for row in reader:
#         medication_name = row['id'].split('-')[-1]
#         print(medication_name)
#         c = Command.objects.get(uuid=row['canvas_externally_exposable_id'])

#         new = f"Name: {medication_name}"
...
#         ms = c.related_objects['anchor_object']
#         ms.sig_original_input = new
#         ms.save()

...
need to fetch a separate URL.

Each template contains a ``__PATIENT_ID__`` sentinel that callers substitute
via ``str.replace()`` — NOT ``str.format()``, which RestrictedPython blocks
in the Canvas plugin sandbox.

The form-submission scripts POST to /plugin-io/api/cms_access_fhir_client/<op>
with the staff session cookie (same-origin), gated by StaffSessionAuthMixin.
"""

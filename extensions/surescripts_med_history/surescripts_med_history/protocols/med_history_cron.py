...
class MedHistoryCronTask(CronTask):
    """Send Surescripts medication history requests for patients with upcoming appointments.
    ...
    def execute(self) -> list[Effect]:
        ...
        log.info(
            "MedHistoryCronTask running for appointment dates %s (offsets %s)"
            % (target_dates, days)
        )
        ...
        for appt in appointments.iterator(chunk_size=200):
            ...
            if not appt.provider.spi_number:
                if provider_id not in warned_providers:
                    ...
                    log.warning(
                        "Skipping Surescripts med history: provider %s has no SPI number"
                        % provider_id
                    )
            ...
            effects.extend(request_metadata_effects(note_id, "med_history"))

        if skipped_no_spi:
            log.info(
                "MedHistoryCronTask skipped %s appointments across %s provider(s) lacking SPI"

...
class TestEligibilityCronTaskNoAppointments:
    @patch("surescripts_med_history.protocols.eligibility_cron.arrow")
    @patch("surescripts_med_history.protocols.eligibility_cron.Appointment")
    def test_returns_empty_when_no_appointments(self, mock_appt_cls, mock_arrow):
        ...
        from surescripts_med_history.protocols.eligibility_cron import (
            EligibilityCronTask,
        )
...
class TestEligibilityCronTaskDualDateRange:
    ...
    def test_multiple_patients_get_separate_effects(self, mock_appt_cls, mock_arrow):
        ...
        prov.spi_number = "1234567"
        appt1 = MagicMock(patient=p1, provider=prov, note=None)
        appt2 = MagicMock(patient=p2, provider=prov, note=None)

...
- **`AttributeHub` flush is not transactional.** On the success path of `commit`, the plugin writes every staged section's UUID map to AttributeHub via per-key `set_attribute` calls. If the DB blips mid-flush (rare, but possible), some sections' UUIDs land in the hub and others don't. The next commit attempt rebuilds the snapshot from the persisted state, sees the partial map, and takes the `edit()` path for sections whose UUIDs were saved + the `originate()` path for sections whose UUIDs weren't. **Chart state still converges correctly** because each section's reconciler is idempotent on its own — `edit()` and `originate()` both produce the right Canvas command. But the per-section paths diverge across one logical commit, which can be confusing if you're tracing a failed-retry timeline. Real fix needs a transactional write API at the AttributeHub layer, which the SDK doesn't currently expose.

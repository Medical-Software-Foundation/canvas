...
from drag_drop_availability.api.events import (
    CalendarEventsAPI,
    _calendar_tz,
...
def test_calendar_tz_no_calendar():
    with patch("drag_drop_availability.api.events.Calendar") as mock_cal:
...
def test_calendar_tz_for_event_no_event():
    with patch("drag_drop_availability.api.events.EventModel") as mock_em:
...
def test_calendar_tz_for_event_returns_tz():
    with patch(
        "drag_drop_availability.api.events.EventModel"
    ) as mock_em, patch(
        "drag_drop_availability.api.events.Calendar"
    ) as mock_cal:
        ev = MagicMock()
...
def test_get_no_tz_param():
    ...
    with patch(
        "drag_drop_availability.api.events.Staff"
    ) as mock_staff, patch(
        "drag_drop_availability.api.events.PracticeLocation"
    ) as mock_loc, patch(
        "drag_drop_availability.api.events.EventModel"
    ) as mock_em, patch(
        "drag_drop_availability.api.events._serialize_event",
...
def test_post_with_body_timezone():
    ...
    with patch("drag_drop_availability.api.events.Event") as mock_event:
...
def test_post_invalid_tz_falls_back_to_calendar():
    ...
    with patch(
        "drag_drop_availability.api.events.Event"
    ) as mock_event, patch(
        "drag_drop_availability.api.events._calendar_tz",

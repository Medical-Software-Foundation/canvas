...
(function () {
    ...
    async function refreshEvents(excludedIds, truncations) {
        try {
            const url = '/plugin-io/api/drag_drop_availability/events?tz=' + encodeURIComponent(state.viewTimezone || 'UTC');
    ...
    async function ensureCalendar(provider, type, locationId) {
        ...
        const result = await apiCall('/plugin-io/api/drag_drop_availability/calendar', {
            method: 'POST',
    ...
    function renderToolbar() {
        ...
        // Split schedulable staff into clinical providers and rooms (role=RR).
        // The server marks each entry with `is_room`. Each gets its own
        ...
        const roomPopover = popoverFor(onlyRooms, 'rooms');
            ...
            el('div', { class: 'right' }, [
                ...
                el('div', { class: 'av-popover-anchor' }, [
                    el('button', {
                        ...
                        onclick: (e) => {
                            e.stopPropagation();
                            state.openFilter = state.openFilter === 'rooms' ? null : 'rooms';
                            renderAll();
                        },
                    }, 'Rooms: ' + filterBtnLabel(onlyRooms, 'rooms') + ' ▾'),
                    roomPopover,
                ]),
        ...
        try {
            if (scope === 'all' || !isRecurring) {
                for (const rec of records) {
                    await apiCall('/plugin-io/api/drag_drop_availability/events', {
                        method: 'DELETE',
    ...
    function renderModal() {
        ...
        const modalChildren = [
            ...
            roomChips.length ? el('div', { class: 'av-form-row' }, [
                el('label', {}, 'Rooms'),
                el('div', { class: 'av-multi' }, roomChips),
            ]) : null,
            isAvail ? el('div', { class: 'av-form-row' }, [

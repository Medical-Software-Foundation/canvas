...
from .conftest import FakeCache, make_request
...
def test_execute_set_returns_404_when_set_id_missing(
    ...
    fake_cache.set("order_sets_data", [])
...
def test_execute_set_calls_helper_with_set_items(
    ...
    fake_cache.set(
        "order_sets_data",
        [{"id": "s1", "items": [item], "order_type": "lab", "lab_partner": "lp"}],
    )
...
def test_execute_order_set_rejects_when_no_items(api_instance: Any) -> None:
    out = api_instance._execute_order_set(
        order_set={"order_type": "lab"}, items=[], patient_id="pt", provider_id=""
    )
...
def test_execute_order_set_falls_back_to_explicit_provider(
    ...
    lab_cmd = mocker.patch(
        "order_sets.api.endpoints.LabOrderCommand"
    )
    lab_cmd.return_value.originate.return_value = MagicMock()

    out = api_instance._execute_order_set(
        order_set={"order_type": "lab", "lab_partner": "lp", "diagnosis_codes": []},
        items=[{"code": "CBC"}],
        patient_id="pt",
        provider_id="prov",
    )
...
def test_execute_order_set_lab_builds_single_command_for_all_codes(
    ...
    # The result list contains the Effect plus the success JSONResponse
    assert len(out) == 2
...
def test_execute_order_set_imaging_creates_one_command_per_item(
    ...
    # One ImagingOrderCommand per item
...
def test_execute_order_set_poc_creates_perform_commands_with_combined_notes(
    ...
    api_instance._execute_order_set(
        order_set={"order_type": "poc", "comment": "in-office", "name": "POC"},
        items=[{"code": "81002", "name": "Urinalysis"}, {"code": "82962", "name": "Glucose"}],
        patient_id="pt-1",
        provider_id="prov",
    )
    # Two PerformCommands, each with notes combining item name and order comment
...
def test_execute_order_set_poc_no_comment_uses_item_name_only(
    ...
    api_instance._execute_order_set(
        order_set={"order_type": "poc", "comment": "", "name": "POC"},

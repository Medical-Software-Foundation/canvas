"""Tests for the Mark inactive audit route on SalesforceStatusAPI.

Covers ``POST /records/<external_id>/mark-inactive``. The route resolves the
linked Canvas patient and drives the Canvas FHIR client to flip ``active`` to
false. Tests inject a fake client through the ``_build_canvas_fhir_client``
seam so they never reach the SDK ``Http`` import.
"""
...
class DeleteRowFactory(factory.django.DjangoModelFactory[IncomingPatientRecord]):
    ...
    email = ""
    phone = ""
    raw_payload = factory.LazyFunction(lambda: {"Id": "00QMI01"})
    content_hash = "hash-mark-inactive-01"
    status = "new"
    received_at = _DELETE_RECEIVED_AT


class FakeFhirClient:
    """Records calls to mark_patient_inactive and lets tests script failures."""
...
def test_mark_inactive_happy_path_flips_row_and_calls_fhir() -> None:
    patient = _seed_linked_patient("00QMI01")
    ...
    responses = _drive(
        ...
        _request(external_id="00QMI01"),
    )

    row = IncomingPatientRecord.objects.get(external_id="00QMI01")
    assert row.status == "accepted"
    assert row.actioned_at is not None
    assert row.actioned_by_id == staff.dbid
    assert fhir.calls == [str(patient.id)]

    entry = ResolutionAuditEntry.objects.get(external_id="00QMI01")
    assert entry.action_taken == "mark_inactive"
    ...
    # Mark inactive talks to the Canvas FHIR API directly, no SDK effect is
    # returned alongside the JSON response.
    assert len(responses) == 1


def test_mark_inactive_409s_when_no_linked_canvas_patient() -> None:
    DeleteRowFactory.create()
    fhir = FakeFhirClient()
...
def test_mark_inactive_404s_when_no_delete_row_for_external_id() -> None:
    responses = _drive(
...
def test_mark_inactive_409s_when_row_already_acted() -> None:
    _seed_linked_patient("00QMI01")
...
def test_mark_inactive_503s_when_canvas_fhir_secret_missing(missing_key: str) -> None:
    _seed_linked_patient("00QMI01")
    DeleteRowFactory.create()
    ...
    body = _json_body(responses[0])
    # The error message names all three secrets so the operator can fix the
...
def test_mark_inactive_502s_with_underlying_message_on_fhir_failure() -> None:
    _seed_linked_patient("00QMI01")
    DeleteRowFactory.create()
    fhir = FakeFhirClient(
...
def test_mark_inactive_502s_on_canvas_fhir_auth_error() -> None:
    _seed_linked_patient("00QMI01")
    DeleteRowFactory.create()
    fhir = FakeFhirClient(

"""Tests for the imaging_app Application handler."""
...
from portal_content.views.imaging_app import ImagingReportsApp


class TestImagingReportsApp:
    """Tests for ImagingReportsApp Application handler."""

    def test_on_open_returns_launch_modal_effect(self):
        """Test that on_open returns LaunchModalEffect when enabled."""
        app = MagicMock(spec=ImagingReportsApp)
        app.secrets = {}
        app.event = MagicMock()
        app.event.context = {"user": {"id": "patient-123"}}

        with patch("portal_content.views.imaging_app.is_component_enabled") as mock_enabled:
            with patch("portal_content.views.imaging_app.log"):
                mock_enabled.return_value = True
                result = ImagingReportsApp.on_open(app)

        mock_enabled.assert_called_once_with("imaging", app.secrets)
        ...
        assert "/plugin-io/api/portal_content/imaging/portal" in str(result)

"""Tests for the visits_app Application handler."""
...
from portal_content.views.visits_app import VisitNotesApp


class TestVisitNotesApp:
    """Tests for VisitNotesApp Application handler."""

    def test_on_open_returns_launch_modal_effect(self):
        """Test that on_open returns LaunchModalEffect when enabled."""
        app = MagicMock(spec=VisitNotesApp)
        app.secrets = {}
        app.event = MagicMock()
        app.event.context = {"user": {"id": "patient-123"}}

        with patch("portal_content.views.visits_app.is_component_enabled") as mock_enabled:
            with patch("portal_content.views.visits_app.log"):
                mock_enabled.return_value = True
                result = VisitNotesApp.on_open(app)

        mock_enabled.assert_called_once_with("visits", app.secrets)
        ...
        assert "/plugin-io/api/portal_content/visits/portal" in str(result)

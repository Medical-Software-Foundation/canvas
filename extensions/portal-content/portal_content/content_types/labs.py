"""Labs content type - lab reports portal."""
...
# FHIR category for lab reports
FHIR_CATEGORY = "labreport"
DEFAULT_REPORT_NAME = "Lab Report"
CONTENT_TYPE = "labs"
...
<body>
    <div class="container">
        ...
        <p class="subtitle">View your lab test results and reports</p>
        <div id="error-message" class="error" style="display: none;"></div>
        <div id="loading" class="loading">Loading your lab reports...</div>
        <div id="empty-state" class="empty-state" style="display: none;">
            <div class="empty-state-icon">🔬</div>
            <h2>No Lab Reports Yet</h2>
            <p>Your lab test results will appear here when they become available.</p>
        </div>
    ...
    <div id="report-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title" id="modal-title">Lab Report</h2>
                <div>
    ...
    <script>
        const API_ENDPOINT = '/plugin-io/api/portal_content/labs/reports';
        ...
        function renderReportDetail(report) {
            ...
            if (report.pdf_url) {
                html = `<div style="width: 100%; height: 100%;">
                    <iframe src="${report.pdf_url}#pagemode=none&view=FitH" style="width: 100%; height: 100%; border: none;" title="Lab Report PDF"></iframe>
                </div>`;
            } else {
                html = `<div style="padding: 30px; overflow-y: auto; height: 100%;"><p>Lab report document not available.</p></div>`;
            }
...
def serve_portal_page(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    log.info(f"Serving labs portal page to patient: {patient_id}")
...
def handle_reports_request(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    patient_id = api.request.headers.get("canvas-logged-in-user-id")
    log.info(f"Labs API request from patient: {patient_id}")
...
def proxy_pdf(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    log.info(f"Proxying labs PDF {document_id} for patient: {patient_id}")
...
def _handle_list(api: "PortalContentAPI", patient_id: str, body: dict) -> list[Response | Effect]:
    """Handle listing lab reports for the patient."""
    try:
        limit = min(int(body.get("limit", 20)), 50)
        offset = int(body.get("offset", 0))

        log.info(f"Listing lab reports for patient {patient_id}, limit={limit}, offset={offset}")
        ...
        if not fhir_client:
            ...
            return [
                JSONResponse(
                    {"status": "error", "message": "Unable to retrieve lab reports"},
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ]
...
def _handle_detail(api: "PortalContentAPI", patient_id: str, body: dict) -> list[Response | Effect]:
    ...
    try:
        ...
        log.info(f"Getting detail for lab report {report_id}")
        ...
        if not fhir_client:
            ...
            return [
                JSONResponse(
                    {"status": "error", "message": "Unable to retrieve lab report"},
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ]
...
def _extract_report_summary(document_reference: dict, patient_id: str) -> dict | None:
    """Extract lab report summary from FHIR DocumentReference resource."""
    try:
        ...
        pdf_url = f"/plugin-io/api/portal_content/labs/pdf?document_id={report_id}"

"""Imaging content type - imaging reports portal."""
...
# FHIR category for imaging reports
FHIR_CATEGORY = "imagingreport"
DEFAULT_REPORT_NAME = "Imaging Report"
...
<head>
    ...
    <title>My Imaging Reports</title>
    <style>
...
<body>
    <div class="container">
        <h1>My Imaging Reports</h1>
        <p class="subtitle">View your imaging test results and reports</p>
        <div id="error-message" class="error" style="display: none;"></div>
        <div id="loading" class="loading">Loading your imaging reports...</div>
        <div id="empty-state" class="empty-state" style="display: none;">
            <div class="empty-state-icon">🏥</div>
            <h2>No Imaging Reports Yet</h2>
            <p>Your imaging test results will appear here when they become available.</p>
        </div>
    ...
    <div id="report-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title" id="modal-title">Imaging Report</h2>
                <div>
    ...
    <script>
        const API_ENDPOINT = '/plugin-io/api/portal_content/imaging/reports';
        ...
        async function loadReports(append = false) {
            ...
            } catch (error) { console.error('Error:', error); showError('We\\'re having trouble loading your reports. Please try again later.'); }
        ...
        async function showReportDetail(reportId) {
            ...
            } catch (error) { console.error('Error:', error); showError('We\\'re having trouble loading this report. Please try again later.'); }
        }
        function renderReportDetail(report) {
            ...
            if (report.pdf_url) {
                html = `<div style="width: 100%; height: 100%;">
                    <iframe src="${report.pdf_url}#pagemode=none&view=FitH" style="width: 100%; height: 100%; border: none;" title="Imaging Report PDF"></iframe>
                </div>`;
            } else {
                html = `<div style="padding: 30px; overflow-y: auto; height: 100%;"><p>Imaging report document not available.</p></div>`;
            }
...
def serve_portal_page(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    log.info(f"Serving imaging portal page to patient: {patient_id}")
...
def handle_reports_request(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    patient_id = api.request.headers.get("canvas-logged-in-user-id")
    log.info(f"Imaging API request from patient: {patient_id}")
...
def proxy_pdf(api: "PortalContentAPI") -> list[Response | Effect]:
    ...
    log.info(f"Proxying imaging PDF {document_id} for patient: {patient_id}")
...
def _handle_list(api: "PortalContentAPI", patient_id: str, body: dict) -> list[Response | Effect]:
    """Handle listing imaging reports for the patient."""
    try:
        limit = min(int(body.get("limit", 20)), 50)
        offset = int(body.get("offset", 0))

        log.info(f"Listing imaging reports for patient {patient_id}, limit={limit}, offset={offset}")
        ...
        if not fhir_client:
            ...
            return [
                JSONResponse(
                    {"status": "error", "message": "Unable to retrieve imaging reports"},
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ]
...
def _handle_detail(api: "PortalContentAPI", patient_id: str, body: dict) -> list[Response | Effect]:
    ...
    try:
        ...
        log.info(f"Getting detail for imaging report {report_id}")
        ...
        if not fhir_client:
            ...
            return [
                JSONResponse(
                    {"status": "error", "message": "Unable to retrieve imaging report"},
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ]
...
def _extract_report_summary(document_reference: dict, patient_id: str) -> dict | None:
    """Extract imaging report summary from FHIR DocumentReference resource."""
    try:
        ...
        pdf_url = f"/plugin-io/api/portal_content/imaging/pdf?document_id={report_id}"

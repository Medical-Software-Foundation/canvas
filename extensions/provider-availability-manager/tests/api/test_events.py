...
from provider_availability_manager.api.events import (
    CalendarEventsAPI,
    _calendar_tz,
...
def test_calendar_tz_for_event_returns_tz():
    with patch(
        "provider_availability_manager.api.events.EventModel"
    ) as mock_em, patch(
        "provider_availability_manager.api.events.Calendar"
    ) as mock_cal:
        ev = MagicMock()
...
def test_get_no_tz_param():
    ...
    with patch(
        "provider_availability_manager.api.events.Staff"
    ) as mock_staff, patch(
        "provider_availability_manager.api.events.PracticeLocation"
    ) as mock_loc, patch(
        "provider_availability_manager.api.events.EventModel"
    ) as mock_em, patch(
        "provider_availability_manager.api.events._serialize_event",
...
def test_post_with_body_timezone():
    ...
    with patch("provider_availability_manager.api.events.Event") as mock_event:
...
def test_post_invalid_tz_falls_back_to_calendar():
    ...
    with patch(
        "provider_availability_manager.api.events.Event"
    ) as mock_event, patch(
        "provider_availability_manager.api.events._calendar_tz",
...
def test_patch_invalid_tz_falls_back():
    ...
    with patch(
        ...
        "provider_availability_manager.api.events._calendar_tz_for_event",
        return_value=ZoneInfo("UTC"),
    ):
        mock_event.return_value.update.return_value = MagicMock(name="effect")
        result = h.patch()

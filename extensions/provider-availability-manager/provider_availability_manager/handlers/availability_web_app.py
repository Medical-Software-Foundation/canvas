...
from provider_availability_manager.api.events import _serialize_event
from provider_availability_manager.utils.staff_lookup import parse_schedulable_roles
from provider_availability_manager.utils.theming import theme_style_block
...
class AvailabilityWebApp(StaffSessionAuthMixin, SimpleAPI):
    ...
    def index(self) -> list[Response | Effect]:
        ...
        # SCHEDULABLE_STAFF_ROLES secret.
        schedulable_roles = parse_schedulable_roles(
            self.secrets.get("SCHEDULABLE_STAFF_ROLES", "")
        )
        providers = (
            Staff.objects
            .filter(active=True, roles__internal_code__in=list(schedulable_roles))

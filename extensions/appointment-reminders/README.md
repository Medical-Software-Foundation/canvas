...
So before querying anything, the scheduler asks whether any configured interval could fire right now, in any timezone a patient can resolve to (see *Patient timezone*). With only day-out intervals configured — a single daily reminder, the common case — that is true on roughly nine ticks a day and the rest return immediately without touching the appointment table. Configure any short reminder interval, or enable telehealth, and every tick scans as before.

The send-time sources the gate considers are the global setting plus any per-visit-type override, each crossed with every resolvable patient zone. Business-line overrides cannot set an interval, send time, or timezone — they only refine copy, channels, and opt-out — so that pair is the complete set. Missing a source here would silently stop that visit type's reminders, so it is covered by tests.
...
Every time the plugin renders — the booking acknowledgement, reminders, the telehealth join message, cancellations, no-shows, manual sends and the admin preview — is rendered in the **patient's** timezone and labeled with the zone (`3:40 PM PT`). Day-out reminders are also *sent* at the configured send time in that same zone, so "09:00" means 09:00 where the patient is.

The zone is resolved in `services/templates.py:resolve_timezone_name`, in this order:

1. **`Patient.last_known_timezone`** — the timezone recorded on the chart, writable through the FHIR Patient `tz-code` extension. This is the override: set it and nothing below is consulted.
2. **The patient's address** (`services/timezones.py`) — state code, refined by three-digit ZIP prefix for the states split across two zones. Prefers an active home address, then any active one, then an inactive one.
3. **The configured default** on the Campaigns tab.
4. **Eastern**, if all of the above are empty or unparseable.

**Step 2 is the one that does the work, and it is why this section exists.** `last_known_timezone` is the only patient timezone column in the schema, but nothing in routine charting writes it: across the production fleet it holds a value on a handful of patient rows in total and is NULL on everything else. A resolver that stopped at step 1 therefore rendered every message in the one configured zone, which is what a patient two zones away saw. Addresses, by contrast, are populated: on the instance this was measured against, 1,340 of 1,353 appointments in the next 30 days had a patient address carrying a state code.

**Accuracy of step 2.** A two-letter state resolves exactly for the 44 states in a single zone, including Arizona, which gets `America/Phoenix` rather than a Denver that would be an hour off for half the year. For split states, the ZIP prefixes in `_SPLIT_STATE_PREFIXES` carry the line where a prefix falls cleanly on one side of it: the Florida panhandle, northwest and southwest Indiana, western Kentucky, east Tennessee, El Paso, western Dakotas, Malheur County, northern Idaho. Where a prefix straddles the line — a few Kentucky border counties, the four Central counties of Michigan's Upper Peninsula, the Mountain corners of Kansas and Nebraska, the Navajo Nation inside Arizona — the state default wins and can be wrong by an hour. Those patients are fixed by writing the real zone to `last_known_timezone`, which outranks everything here. A non-US address resolves to nothing and falls through to the configured default rather than being placed on a US zone.

**What the scan gate costs.** Because a day-out reminder now fires at its send time in the patient's zone rather than the instance's, the pre-query gate has to admit any tick that is the send time in *any* resolvable zone. That is about nine US zones instead of one, so a single daily reminder opens roughly nine scans a day rather than one. The other ~279 ticks still return without touching the appointment table, and `_is_day_out_window` still makes the exact per-appointment decision.
...
- **A visit type stores "inherit" as an absent key, not `true`.** The resolver treats absent and `true` identically — only an explicit `false` opts a visit type out — so the admin app writes only `false`. Records written before v0.16.0 carry explicit `true` values, which resolve as inherit and are harmless; no migration is needed. Note that Save gathers every tab, so a Save from Campaigns still rewrites `note_type_reminders`; it is now a no-op rather than a pin.
- **Address-derived timezones are state-level, refined by ZIP prefix.** Resolution is a lookup table, not a geocode, so a patient in one of the border counties a three-digit prefix straddles can be an hour off — see *Patient timezone* for the specific ones. The escape hatch is per patient rather than global: write the real zone to `last_known_timezone` through the FHIR `tz-code` extension and it outranks the table. A finer table would need ZIP5 data the plugin sandbox has no room to carry.
- **The config record has no audit trail.** `CampaignConfigRecord` is a single JSON row with no `modified` column and no history, so a change that silences a campaign cannot be attributed or dated from the config itself — only inferred from gaps in the delivery log. Worth addressing for a surface that can stop patient messaging fleet-wide.
...
- **`{{location_phone}}` and `{{organization_phone}}` are formatted for patients.** Contact points store bare digits, so an unformatted value reached the patient as "Call 8005550199". North American numbers render as `(800) 555-0199`; anything else is passed through unchanged rather than risk mangling an international number.

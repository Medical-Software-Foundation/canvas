(function () {
    ...
    var isAdmin = adminApp.dataset.isAdmin === "1";
    ...
    function bindEvents() {
        ...
        // Order type toggle: clear items first (they don't carry across types),
        // then update the visible subform.
        document.getElementById("form-order-type").addEventListener("change", function () {
            selectedItems = [];
    ...
    async function loadSets() {
        ...
        try {
            ...
            // Guard against the server returning JSON that isn't an array
            // (e.g. an error envelope with a 200, or a different content type).
            if (!Array.isArray(allSets)) {
                allSets = [];
                showToast("Failed to load order sets (unexpected shape)", "error");
            }
            renderAdminSets();
        } catch (e) {
    ...
    async function loadLabPartners() {
        try {
            ...
            if (!resp.ok) {
                showToast("Failed to load lab partners (server error " + resp.status + ")", "error");
                labPartners = [];
                return;
            }
            var data = await resp.json();
            labPartners = Array.isArray(data) ? data : [];
            ...
            labPartners = [];
            showToast("Failed to load lab partners: " + e.message, "error");
        }
    }
    ...
    async function searchTests() {
        ...
        try {
            ...
            if (!resp.ok) {
                resultsDiv.innerHTML = '<div class="test-result-item" style="color:#9CA3AF">Search failed (server error ' + resp.status + ')</div>';
                show(resultsDiv);
                return;
            }
            ...
            resultsDiv.innerHTML = '<div class="test-result-item" style="color:#9CA3AF">Search failed: ' + escapeHtml(e.message) + '</div>';
            show(resultsDiv);
        }
    }
    ...
    async function searchCPT() {
        ...
        try {
            ...
            var resp = await fetch(url, { credentials: "same-origin" });
            if (!resp.ok) {
                resultsDiv.innerHTML = '<div class="test-result-item" style="color:#9CA3AF">CPT search failed (server error ' + resp.status + ')</div>';
                show(resultsDiv);
                return;
            }
            var tests = await resp.json();
            ...
            resultsDiv.innerHTML = '<div class="test-result-item" style="color:#9CA3AF">CPT search failed: ' + escapeHtml(e.message) + '</div>';
            show(resultsDiv);
        }
    }
    ...
    function renderAdminSets() {
        ...
        adminSetsList.innerHTML = allSets.map(function (s) {
            ...
            // Authorization mirrors the server-side _can_modify rule:
            // - Personal sets: only the creator may modify
            // - Shared sets: only admins may modify
            var canModify = s.is_shared ? isAdmin : (s.created_by === staffId);
            var actions = canModify
                ? '<button class="btn btn-secondary btn-sm" data-action="edit" data-id="' + escapeHtml(s.id) + '">Edit</button>' +
                  '<button class="btn btn-danger btn-sm" data-action="delete" data-id="' + escapeHtml(s.id) + '">Delete</button>'
    ...
    function toggleOrderTypeFields() {
        ...
        hide(document.getElementById("test-results"));
        hide(document.getElementById("cpt-results"));
        // The browser runs constraint validation on hidden-but-required fields
        // and blocks form submit before our listener runs. Toggle `required`
        // to match visibility so imaging/POC submits can fire.
        document.getElementById("form-lab-partner").required = (orderType === "lab");
        // Hide Dx codes for POC orders — PerformCommand doesn't accept them,
        // so we don't want the UI to suggest they apply.
        var dxGroup = document.getElementById("form-diagnosis-group");
        if (dxGroup) {
            if (orderType === "poc") hide(dxGroup); else show(dxGroup);
        }
        if (orderType === "lab") {
    ...
    async function saveSet() {
        ...
        if (orderType === "lab") {
            ...
            if (!labPartnerId) {
                showToast("Select a lab partner", "error");
                return;
            }

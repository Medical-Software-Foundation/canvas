...
from provider_scheduling.handlers.availability_web_app import AvailabilityWebApp


class DummyRequest:
    """A dummy request object for testing AvailabilityWebApp."""
...
def test_web_app_prefix_configuration() -> None:
    ...
    assert AvailabilityWebApp.PREFIX == "/app"
...
def test_index_queries_active_clinical_providers() -> None:
    ...
    with (
        patch("provider_scheduling.handlers.availability_web_app.Staff") as mock_staff_class,
        patch("provider_scheduling.handlers.availability_web_app.StaffRole") as mock_staff_role,
        patch("provider_scheduling.handlers.availability_web_app.PracticeLocation") as mock_location_class,
        patch("provider_scheduling.handlers.availability_web_app.NoteType") as mock_note_type_class,
        patch("provider_scheduling.handlers.availability_web_app.Event") as mock_event_class,
        patch("provider_scheduling.handlers.availability_web_app.render_to_string") as mock_render,
...
def test_index_includes_calendar_types_in_context() -> None:
    ...
    with (
        ...
        assert context["calendarTypes"][0]["label"] == "Available"
        assert context["calendarTypes"][1]["label"] == "Busy"
...
def test_get_main_js_returns_javascript() -> None:
    ...
    # Create web app instance
    dummy_context = {"method": "GET", "path": "/app/availability.js"}
    app = AvailabilityWebApp(event=DummyEvent(context=dummy_context))  # type: ignore[arg-type]

    with patch("provider_scheduling.handlers.availability_web_app.render_to_string") as mock_render:
...
def test_get_main_js_renders_correct_template() -> None:
    ...
    with patch("provider_scheduling.handlers.availability_web_app.render_to_string") as mock_render:
        ...
        mock_render.assert_called_once_with("static/availability/main.js")


def test_get_css_returns_stylesheet() -> None:
    """Test get_css endpoint returns CSS with correct content type."""
    # Create web app instance
    dummy_context = {"method": "GET", "path": "/app/availability.css"}
    app = AvailabilityWebApp(event=DummyEvent(context=dummy_context))  # type: ignore[arg-type]
...
def test_get_css_renders_correct_template() -> None:
    ...
    with patch("provider_scheduling.handlers.availability_web_app.render_to_string") as mock_render:
        ...
        # Verify render_to_string was called with correct template
        mock_render.assert_called_once_with("static/availability/styles.css")

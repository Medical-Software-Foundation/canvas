...
class TestAuthenticate:
    ...
    def test_false_for_patient_session(self):
        handler = _make_handler()
        creds = MagicMock()
        creds.logged_in_user = {"type": "Patient", "id": "pat-1"}
        assert handler.authenticate(creds) is False

    def test_false_when_logged_in_user_none(self):
        handler = _make_handler()
        creds = MagicMock()
        creds.logged_in_user = None
        assert handler.authenticate(creds) is False

    def test_false_for_staff_without_id(self):
        handler = _make_handler()
        creds = MagicMock()
        creds.logged_in_user = {"type": "Staff", "id": ""}
        assert handler.authenticate(creds) is False

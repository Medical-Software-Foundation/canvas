...
class SupervisorCoSignAPI(SimpleAPI):
    def authenticate(self, credentials: SessionCredentials) -> bool:
        user = credentials.logged_in_user or {}
    ...
    def submit_cosign(self) -> list[Response | Effect]:
        ...
        for r in pending:
            ...
            if r.task_id:
                ...
                if additional_comments:
                    comment_body += f"\n\n{additional_comments}"
                log.info(f"supervisor_cosign.submit: firing AddTaskComment + UpdateTask(COMPLETED) for task_id={r.task_id!r}")
        ...
        effects.append(JSONResponse({"status": "approved", "count": len(pending)}, status_code=HTTPStatus.OK))
        return effects

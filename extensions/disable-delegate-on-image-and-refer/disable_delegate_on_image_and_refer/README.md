...
By default the Image and Refer commands offer both a **Sign** and a **Delegate** action, so a provider can hand an imaging order or referral off to another staff member to sign. Some practices want their providers to take personal responsibility for every imaging order and referral they place. Without this plugin the only way to enforce that is staff training and after-the-fact auditing; the plugin removes the Delegate affordance entirely so signing personally is the only path.

## Who it's for

Practices and specialties that require the ordering provider to personally sign their own imaging orders and referrals rather than delegating them — for example, practices with compliance or attribution policies around who signs an order. It applies to any role that places Image or Refer commands.

## Screenshots

An Image (Imaging Order) command with the plugin installed — only the **Sign** action is offered, with no **Delegate** button:

![Image command showing only the Sign action, no Delegate](screenshots/image-command-sign-only.png)

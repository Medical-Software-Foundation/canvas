  ...
  */
  ...
  class CanvasMultiSelect extends HTMLElement {
    ...
    _readOptions() {
      ...
      for (var i = 0; i < opts.length; i++) {
        var opt = opts[i];
        var val = opt.hasAttribute('value') ? opt.getAttribute('value') : opt.textContent.trim();
        this._options.push({
          value: val,
          label: opt.getAttribute('label') || opt.textContent.trim(),
          html: opt.innerHTML,
    ...
    _render() {
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          /* The same checkbox canvas-checkbox draws, to the pixel. A row in this
             menu and a standalone checkbox are the same control doing the same
             job, so they cannot look like two different controls. Drawn with
             pseudo elements rather than by nesting the component itself,
             because the row is the option and a second focusable control inside
             it would break both the keyboard walk and the selection semantics. */
          .option::before {
            content: '';
            position: absolute;
            left: 1em;
          ...
          /* In canvas-checkbox the tick is positioned inside the box, so its
             1px border pushes the tick a pixel down and right before the box's
             own offsets apply. Here the tick is positioned against the row
             instead, because a pseudo element cannot sit inside another one, so
             that border has to be added back by hand. Both numbers are the box
             offset plus the border plus canvas-checkbox's own 1px and 4px. */

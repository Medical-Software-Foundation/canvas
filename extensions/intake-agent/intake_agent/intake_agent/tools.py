...
from canvas_sdk.v1.data.patient import Patient as PatientModel
...
"""
    ...
    @classmethod
    def create_patient(cls, session: IntakeSession) -> Effect:
        # Create patient effect with phone contact
        patient_effect = Patient(
            first_name=session.first_name,
            last_name=session.last_name,
            birthdate=session.date_of_birth,
            contact_points=[
                PatientContactPoint(
                    ...
                    rank=1,
                    has_consent=session.phone_verified()
                ),
            ],

...
class PregnantPersonsNotPreviouslyDiagnosedWithDiabetes(ValueSet):
    VALUE_SET_NAME = 'Pregnant persons who have not been previously diagnosed with type 1 or type 2 diabetes'
    SNOMEDCT = {'169826009', '199227004', '237618001', '77386006', '72892002'}
    ICD10CM = {'O24.410', 'O24.414', 'O24.419', 'Z34.00', 'Z34.90'}

class AsymptomaticPregnantPersonsAfter24Weeks(ValueSet):
    VALUE_SET_NAME = 'Asymptomatic pregnant persons at 24 weeks of gestation or after'
    SNOMEDCT = {'16932000', '714035009', '714036005', '714037001', '714038006'}
    ICD10CM = {'O09.891', 'O09.892', 'O09.893', 'Z3A.24', 'Z3A.25'}
...
class TerminalIllnessValueSet(ValueSet):
    pass

class RecentSurgeryValueSet(ValueSet):
    pass

def in_numerator(patient: Patient) -> bool:
    severe_allergies = patient.allergy_intolerances.find(SevereAllergyValueSet)
    if severe_allergies:
        return False
    terminal_illness = patient.conditions.find(TerminalIllnessValueSet)
    if terminal_illness:
        return False
    thirty_days_ago = arrow.now().shift(days=-30)
    recent_surgery = patient.procedures.after(thirty_days_ago)
    if recent_surgery:
        return False
    return True
class ComposerResult:
...
class AsymptomaticPregnantPersons(ValueSet):
    VALUE_SET_NAME = 'Asymptomatic Pregnant Persons Without Previous Diabetes Diagnosis'
    SNOMEDCT = {'237630007', '72892002', '77386006'}
    ICD10CM = {'Z34.00', 'Z34.80', 'Z34.90'}

class ScreeningForGestationalDiabetes(ValueSet):
    VALUE_SET_NAME = 'Screening for Gestational Diabetes'
    CPT = {'82947', '82950', '82951'}
    LOINC = {'1558-6', '1554-5', '14771-0'}
    ICD10PCS = {'7G31XZZ'}

class ScreeningFrequency(ValueSet):
    VALUE_SET_NAME = 'Screening Frequency for Gestational Diabetes'
    SNOMEDCT = {'30766002'}
...
def recommendations(patient: Patient) -> ComposerResult:
    ...
    if patient.is_female:
        ...
        if current_gestational_week >= 24:
            result.add_recommendation(title='Screening for Gestational Diabetes', description='Recommendation to perform screening for gestational diabetes as the patient is at or beyond 24 weeks of gestation.')
    return result

class GestationalDiabetesScreeningRecommendation(ClinicalQualityMeasure):
    class Meta:
        title = "Gestational Diabetes Screening"
        description = "USPSTF recommendations for gestational diabetes screening in pregnancy"
        version = "v20240528121021"
        information = 'https://www.canvasmedical.com/'
        identifiers = ["AUTO20240528121021"]
        types = ['CQM']

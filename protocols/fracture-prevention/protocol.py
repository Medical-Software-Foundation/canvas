...
class CommunityDwellingAsymptomaticAdults(ValueSet):
    VALUE_SET_NAME = 'Community-Dwelling, Asymptomatic Adults'
    SNOMEDCT = {'160245001', '224960004', '133931000119108'}

class NotLivingInNursingHomeOrOtherInstitutionalCareSetting(ValueSet):
    VALUE_SET_NAME = 'Not Living in a Nursing Home or Other Institutional Care Setting'
    SNOMEDCT = {'83891005', '257500003'}

def in_denominator(patient: Patient) -> bool:
    return False
class ExcludedPatients(ValueSet):
    VALUE_SET_NAME = 'ExcludedPatients'
    ICD10CM = {'Z00.00', 'Z02.5', 'Z02.6', 'Z76.2', 'Z02.89'}
    SNOMEDCT = {'183932001', '183945008', '183964008', '183949004'}
...
def recommendations(patient: Patient) -> ComposerResult:
    ...
    result.add_task(title='Check Flu Vaccination Status', description="Review patient's immunization records to ensure annual flu vaccination is up to date.")
    result.add_task(title='Monitor Blood Pressure', description='For patients with a history of hypertension, ensure regular monitoring of blood pressure.')
    result.add_task(title='Review Diabetes Management Plan', description='For diabetic patients, review and update the diabetes management plan as necessary.')
    return result

class FracturePreventionGuidelinesUSPSTF(ClinicalQualityMeasure):
    class Meta:
        title = "Fracture Prevention Guidelines"
        description = "USPSTFs guidelines on vitamin D and calcium supplementation for fracture prevention in adults"
        version = "v20240606031042"
        information = 'https://www.canvasmedical.com/'
        identifiers = ["AUTO20240606031042"]
        types = ['CQM']

...
# Replace with the ID of your phone call disposition questionnaire
PHONE_CALL_DISPOSITION_QUESTIONNAIRE_ID = 'QUES_PHONE_01'
# Replace with your task type
APPOINTMENT_RESCHEDULE_TASK_TYPE = 'Appointment Reschedule'
...
class AppointmentReschedule(ClinicalQualityMeasure):
    class Meta:
    ...
    def in_denominator(self) -> bool:
        '''
        Check for patients with an "appointment reschedule" task.

        Returns:
            bool: True if the patient has an "appointment reschedule" task, False otherwise.
        '''
        return bool(self._get_tasks_by_label([APPOINTMENT_RESCHEDULE_TASK_TYPE]))
    ...
    def compute_results(self) -> ProtocolResult:
        '''
        Computes the results of the appointment reschedule protocol.
        ...
        if self.in_denominator():
            if self.in_numerator():
                ...
                result.add_narrative(
                    (
                        'Patient has a task to reschedule their appointment. '
                        'Contact them to reschedule their appointment.'
                    )
                )
        else:
            result.status = STATUS_NOT_APPLICABLE
            result.add_narrative('Patient does not have an appointment to reschedule.')

...
Surfaces a protocol card for every patient who has an active asthma condition
on file, recommending that the patient be contacted. Patients without an active
asthma condition receive a satisfied card.
"""
...
class Ccp005v1(ClinicalQualityMeasure):
    ...
    class Meta:
        ...
        default_permission_flags = {"protocols:actions:CCP005v1:": True}
    ...
    @cached_property
    def active_asthma_conditions(self) -> list[Condition]:
        """Active asthma conditions for the patient, oldest first by onset."""
        return list(
    ...
    def date_of_diagnosis(self) -> str:
        """The earliest onset date among the patient's active asthma conditions, or ''."""
        for condition in self.active_asthma_conditions:
    ...
    def compute(self) -> list[Effect]:
        """Emit a single ProtocolCard effect describing the patient's asthma status."""
        ...
        if self.in_numerator():
            ...
            card.narrative = (
                f"{patient.first_name} has been diagnosed of asthma on {formatted_date}."
            )
            ...
            card.narrative = f"{patient.first_name} has not been diagnosed of asthma."
